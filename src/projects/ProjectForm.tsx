...
function ProjectForm({ project: initialProject, onSave, onCancel }: ProjectFormProps) {
  ...
  const handleChange = (event: any) => {
      ...
      //if input type is number convert the updatedValue string to anumber
      if (type === 'number') {
      ...
      // so we can keep the project properties that aren't being editedlike project.id
  ...
  return (
    <form className="input-group vertical" onSubmit={handleSubmit}>
      ...
      <input type="text" name="name" placeholder="enter name" value={project.name} onChange={handleChange} />{errors.name.length > 0 && (
